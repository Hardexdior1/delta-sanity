...
console.log(reports)
  ...
  useEffect(() => {
    const fetchReport = async () => {
      try {
        setLoading(true);
        const res = await endpointroute.get(`reports`);
        setReport(res.data);
        setNotFound(false);
      } catch (error) {
...
const handlePrintRequest = async (e) => {
  ...
  try {
    ...
    // Message to user
    setResponseMsg(
      `✅ Report containing all submissions from ${format(
        new Date(fromDate),
        "MMMM d, yyyy"
...
>
  <div className="flex flex-col gap-2">
    <h4 className="text-blue-600 font-bold text-lg">
                Submitted by {report.user.fullName}
              </h4>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50">
          <div className="bg-white max-h-[90vh] overflow-y-auto p-6 rounded-lg w-11/12 xl:w-3/5 shadow-xl relative print-area">
            <div className="flex justify-between items-center ">
              <h4 className="text-blue-600 font-bold text-lg">
                Submitted by {view.user.fullName}
              </h4>
