...
const [id,setId]=useState()
  ...
  const handleDeleteConfirmed = async () => {
    try {
      setDeleting(true)
      await endpointroute.delete(`rooms/${selectedRoom._id}`);
  ...
  return (
    <main className="">
      <h3 className="text-xl font-semibold text-emerald-600 mb-4">All Rooms</h3>
<ToastContainer />
      {loading ?  (
  ...
  >
    Next
      ...
      {isDeleteModalOpen && selectedRoom && (
        <ConfirmDeleteModal
          title={`Are you sure you want to delete "${selectedRoom.name}"?`}
